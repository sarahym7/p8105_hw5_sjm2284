...
baltimore_homicide = homicide_raw_data %>% 
  ...
  rowwise() %>% 
  mutate(
  test_result = list( prop.test (x = total_unsovled , n =  total_homicides)),
  estimate = test_result$estimate,
  lower_limit = test_result$conf.int[1],
  upper_limit = test_result$conf.int[2]) %>% 
  unnest(c(estimate, lower_limit, upper_limit)) %>% 
