...
```
Based  on the graph we can see that as effect size increases so does power. There is a positive association until we reach mu=5 and mu =6 where power starts to plateau and even out. This association is true because if we were to increase effect size this will allow for a type 1 error to occur ( rejecting the null when it should be failed).
...
The raw homicide data contains `r ncol(homicide_raw_data)` variables and `r nrow(homicide_raw_data)` observations. Additionally, the names of the variables of interest are `r names(homicide_raw_data )`. 
...
The raw homicide data contains with city_state contains `r ncol(homicide_raw_data)` variables and `r nrow(homicide_raw_data)` observations. Additionally, the names of the variables of interest are `r names(homicide_raw_data )`. 
...
From the graph we see that Tulsa, AL has the lowest proportion of unsolved homicides and a very wide confidence interval. Then we can see that Chicago, IL has the highest proportion of unsolved homicides and this can be held true given that Chicago has a high crime and homicide rate. 
